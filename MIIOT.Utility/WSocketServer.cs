...
namespace MIIOT.Utility
{
    ...
    public class WSocketServer : IDisposable
    {
        ...
        #region 向外传递数据事件
        public event Action<WebSocketVersion, string> MessageReceived;
        ...
        public bool Open(int port, string serverName, bool isUseCertificate = false, string serverStoreName = "", string serverSecurity = "", string serverThumbprint = "")
        {
            ...
            try
            {
                ...
                if (isSetuped)
                {
                    Log.WriteInfo("Setup Success...");
                }
                else
                {
                    Log.WriteInfo("Failed to setup!");
                }
                ...
                if (isSetuped)
                {
                    Log.WriteInfo("Start Success...");
                    Log.WriteInfo("Server Listen at " + this.WebSocket.Listeners[0].EndPoint.Port.ToString());
                ...
                {
                    Log.WriteInfo("Failed to start!");
                }
            }
            catch (Exception ex)
            {
              Log.WriteError (ex.ToString(),ex);
            }
            return isSetuped;
        }
        /// <summary>
        ...
        void NewMessageReceived(WebSocketSession session, string value)
        {
            try
            {
                Log.WriteInfo("Receive:" + value.ToString() + " ClientIP:" + session.RemoteEndPoint);
        ...
        void ProcessMaintainance()
        {
            do
            {
                try
                {
                    Log.WriteInfo("Display Session Info:" + this.WebSocket.SessionCount);
                    foreach (WebSocketSession session in this.WebSocket.GetAllSessions())
                    {
