...
namespace MIIOT.DiagManager.Core.Comm
{
    public static class Extend
    {
        ...
        public static T GetEnum<T>(this int t)
        {
            bool isInEnum = false;
            T c = default(T);
            if (t > 0)
            {
                foreach (int i in Enum.GetValues(typeof(T)))
                {
                    if (i == t)
                    {
                        c = (T)Enum.Parse(typeof(T), i.ToString());
                        isInEnum = true;
                        return c;
                    }
                }
        ...
        private static byte[] GBStringToBytes(this string TheString)
        {
        ...
        private static string BytesToGBString(this byte[] Bytes)
        {
            string Mystring;
        ...
        public static string ByteToHexStr(this byte[] bytes, string delimiter = "")
        {
            ...
            try
            {
                if (bytes != null)
                {
                    returnStr = bytes.Aggregate(returnStr, (current, t) => current + t.ToString("X2") + delimiter);
                }
            }
            catch (Exception ex)
            {
        ...
        public static byte CalcationCRCSum(this byte[] Instruct)
        {
            try
            {
                var nSum = 0;
        ...
        /// <returns></returns>
        public static byte CalcationCheckout(this byte[] Instruct, int beginPos = 0)
        {
            try
            {
                byte nSum = 0x00;
        ...
        public static int CRC16_XMODEM(this Byte[] data)
        {
        ...
        public static DateTime UnixToDateTime(this DateTime target, long timestamp)
        {
            ...
            var temp = start.AddMilliseconds(timestamp);
        ...
        /// <returns></returns>
        public static Type GetTypeByStr(this string typeName)
        {
            Type type = null;
        ...
        public static byte[] StructToBytes(this object structObj, int size)
        {
        ...
        /// 将byte数组转换为结构体
        /// </summary>
        public static object ByteToStruct(this byte[] bytes, Type type)
        {
